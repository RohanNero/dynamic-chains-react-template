...
export const fetchPrice = async (chainId: number): Promise<number> => {
  // get symbol from chains, get native currency contract from chainData
  let symbol = "";
  Object.entries(chains).forEach(([chainKey, chainValue]) => {
    if (chainId === chainValue.id) {
      symbol = chainValue.nativeCurrency.symbol;
    }
  });
  const currencyAddress = chainData[chainId]?.nativeCurrencyTokenAddress;
